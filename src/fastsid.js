...
export default class FastSID {
  constructor(opts) {
    ...
    this.clock_rate = opts.clock || chip.clock.PAL;
  ...
  setup_wavetables() {
    ...
    for (i = 0; i < 4096; i++) {
      ...
      if (this.sid_model == chip.model.MOS8580) {
  ...
  calculate_samples(pbuf, nr, interleave, delta_t, offset) {
    var i;
    for (i = 0; i < nr; i++) {
