...
export default class TinySID {
  constructor(opts) {
    ...
    this.filt_mul = Math.floor(convertFromFloat(21.5332031) / this.mix_freq);
  ...
  generateIntoBuffer(count, buffer, offset) {
    ...
    for (bp = offset; bp < endbp; bp += 1) {
      ...
      this.filter.h = convertFromInt(outf) - (this.filter.b >> 8) * this.filter.rez - this.filter.l;
      this.filter.b += multiply(this.filter.freq, this.filter.h);
      this.filter.l += multiply(this.filter.freq, this.filter.b);

      outf = 0;
      if (this.filter.l_ena) outf += convertToInt(this.filter.l);
      if (this.filter.b_ena) outf += convertToInt(this.filter.b);
      if (this.filter.h_ena) outf += convertToInt(this.filter.h);
...
class Filter {
  ...
  precalc() {
    ...
    if (this.freq > convertFromInt(1)) {
      this.freq = convertFromInt(1);
    }
    this.l_ena = get_bit(this.sid.ftp_vol, 4);
    this.b_ena = get_bit(this.sid.ftp_vol, 5);
    this.h_ena = get_bit(this.sid.ftp_vol, 6);
    this.v3ena = !get_bit(this.sid.ftp_vol, 7);
    this.vol = (this.sid.ftp_vol & 0xf);
    this.rez = convertFromFloat(1.2) - convertFromFloat(0.04) * (this.sid.res_ftv >> 4);
    this.rez >>= 8;
  }
}
...
class Osc {
  constructor(sidinstance, voicenum) {
    this.sid = sidinstance;
    this.vnum = voicenum;
  ...
  precalc() {
    ...
    this.filter = get_bit(this.sid.res_ftv, this.vnum);
  ...
  sampleUpdate() {
    ...
    if (this.noisepos != (this.counter >> 23)) {
      ...
      (get_bit(this.noiseval, 22) ^ get_bit(this.noiseval, 17));
      this.noiseout =
        (get_bit(this.noiseval, 22) << 7) |
      (get_bit(this.noiseval, 20) << 6) |
      (get_bit(this.noiseval, 16) << 5) |
      (get_bit(this.noiseval, 13) << 4) |
      (get_bit(this.noiseval, 11) << 3) |
      (get_bit(this.noiseval, 7) << 2) |
      (get_bit(this.noiseval, 4) << 1) |
      (get_bit(this.noiseval, 2) << 0);
    }
    if (this.wave & 0x04) {
...
function convertFromInt(i) {
  return (i << 16) & 0xffffffff;
}

function convertFromFloat(f) {
