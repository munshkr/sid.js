...
export default class ReSID {
  constructor(opts) {
    opts = opts || {};
    this.sid_model = opts.model || chip.model.MOS6581;
    var clkRate = opts.clock || chip.clock.PAL;
    var sampleRate = opts.sampleRate || 44100;
    var method = opts.sample || sampling_method.SAMPLE_FAST;
  ...
  set_sampling_parameters(clock_freq, method, sample_freq, pass_freq, filter_scale) {
    ...
    if (method == sampling_method.SAMPLE_RESAMPLE_INTERPOLATE || method == sampling_method.SAMPLE_RESAMPLE_FAST) {
      if (constants.FIR_N * clock_freq / sample_freq >= constants.RINGSIZE) {
        return false;
      }
    }
    if (pass_freq < 0) {
    ...
    this.cycles_per_sample = Math.floor(clock_freq / sample_freq * (1 << constants.FIXP_SHIFT) + 0.5);
    ...
    var res = (method == sampling_method.SAMPLE_RESAMPLE_INTERPOLATE) ? constants.FIR_RES_INTERPOLATE : constants.FIR_RES_FAST;
    ...
    for (var i = 0; i < this.fir_RES; i++) {
      ...
      for (var j = -this.fir_N / 2; j <= this.fir_N / 2; j++) {
        ...
        var val = (1 << constants.FIR_SHIFT) * filter_scale * f_samples_per_cycle * wc / Math.PI * sincwt * Kaiser;
    ...
    if (!this.sample) {
      this.sample = new Array(constants.RINGSIZE * 2);
    }
    // Clear sample buffer.
    for (var k = 0; k < constants.RINGSIZE * 2; k++) {
      this.sample[k] = 0;
    }
    this.sample_index = 0;
    return true;
  ...
  adjust_sampling_frequency(sample_freq) {
    ...
    this.cycles_per_sample = Math.floor(this.clock_frequency / sample_freq * (1 << constants.FIXP_SHIFT) + 0.5);
  }
  ...
  clock(delta_t, buf, n, interleave, buf_offset) {
    interleave = interleave || 1;
    buf_offset = buf_offset || 0;
    switch (this.sampling) {
    default:
    case sampling_method.SAMPLE_FAST:
      return this.clock_fast(delta_t, buf, n, interleave, buf_offset);
    case sampling_method.SAMPLE_INTERPOLATE:
      return this.clock_interpolate(delta_t, buf, n, interleave, buf_offset);
    case sampling_method.SAMPLE_RESAMPLE_INTERPOLATE:
      return this.clock_resample_interpolate(delta_t, buf, n, interleave, buf_offset);
    case sampling_method.SAMPLE_RESAMPLE_FAST:
      return this.clock_resample_fast(delta_t, buf, n, interleave, buf_offset);
    }
  }

  clock_fast(delta_t, buf, n, interleave, buf_offset) {
    var s = 0;
    for (;;) {
      var next_sample_offset = this.sample_offset + this.cycles_per_sample + (1 << (constants.FIXP_SHIFT - 1));
      var delta_t_sample = next_sample_offset >> constants.FIXP_SHIFT;
      ...
      this.sample_offset = (next_sample_offset & constants.FIXP_MASK) - (1 << (constants.FIXP_SHIFT - 1));
    ...
    this.sample_offset -= delta_t << constants.FIXP_SHIFT;
    delta_t = 0;
    return s;
  }

  clock_interpolate(delta_t, buf, n, interleave, buf_offset) {
    var s = 0;
    var i;
    for (;;) {
      ...
      delta_t -= delta_t_sample;
      this.sample_offset = next_sample_offset & constants.FIXP_MASK;

      var sample_now = this.output();
        // new sample output w/ offset
      var final_sample = parseFloat(this.sample_prev + (this.sample_offset * (sample_now - this.sample_prev) >> constants.FIXP_SHIFT)) / 32768;
      var buf_idx = s++ * interleave + buf_offset;
      buf[buf_idx] = final_sample;
  ...
  clock_resample_interpolate(delta_t, buf, n, interleave, buf_offset) {
    var s = 0;
    for (;;) {
      var next_sample_offset = this.sample_offset + this.cycles_per_sample;
      var delta_t_sample = next_sample_offset >> constants.FIXP_SHIFT;
      ...
      for (var i = 0; i < delta_t_sample; i++) {
        ...
        this.sample[this.sample_index + constants.RINGSIZE] = this.sample[this.sample_index];
        ++this.sample_index;
      ...
      var fir_offset = this.sample_offset * this.fir_RES >> constants.FIXP_SHIFT;
      var fir_offset_rmd = this.sample_offset * this.fir_RES & constants.FIXP_MASK;
      var fir_start = fir_offset * this.fir_N;
      var sample_start = this.sample_index - this.fir_N + constants.RINGSIZE;
      ...
      var v = v1 + (fir_offset_rmd * (v2 - v1) >> constants.FIXP_SHIFT);
      v >>= constants.FIR_SHIFT;
  ...
  clock_resample_fast(delta_t, buf, n, interleave, buf_offset) {
    var s = 0;
    for (;;) {
      var next_sample_offset = this.sample_offset + this.cycles_per_sample;
      var delta_t_sample = next_sample_offset >> constants.FIXP_SHIFT;
  ...
  generateIntoBuffer(count, buffer, offset) {
    //console.log("generateIntoBuffer (count: " + count + ", offset: " + offset + ")");
    // FIXME: this could be done in one pass. (No?)
    for (var i = offset; i < offset + count; i++) {
      buffer[i] = 0;
    }
    var delta = (this.cycles_per_sample * count) >> constants.FIXP_SHIFT;
    var s = this.clock(delta, buffer, count, 1, offset);
    //console.log("generateIntoBuffer (delta: " + delta + ", samples clocked: " + s + ")");
    return s;
  }
...
class EnvelopeGenerator {
  ...
  reset() {
    ...
    this.rate_period = EGRateCounterPeriod[this.release];
    this.hold_zero = true;
  }
  ...
  writeCONTROL_REG(control) {
    ...
    if (!this.gate && gate_next) {
      ...
      this.rate_period = EGRateCounterPeriod[this.attack];
  ...
  writeATTACK_DECAY(attack_decay) {
    this.attack = (attack_decay >> 4) & 0x0f;
    this.decay = attack_decay & 0x0f;
    if (this.state == EGState.ATTACK) {
      this.rate_period = EGRateCounterPeriod[this.attack];
    } else if (this.state == EGState.DECAY_SUSTAIN) {
      this.rate_period = EGRateCounterPeriod[this.decay];
    }
  }

  writeSUSTAIN_RELEASE(sustain_release) {
  ...
  clock_common() {
    this.rate_counter = 0;
    if (this.state == EGState.ATTACK || ++this.exponential_counter == this.exponential_counter_period) {
      this.exponential_counter = 0;
      if (this.hold_zero) {
        return;
      }
      ...
      case EGState.ATTACK:
        ...
        if (this.envelope_counter == 0xff) {
          this.state = EGState.DECAY_SUSTAIN;
          this.rate_period = EGRateCounterPeriod[this.decay];
        }
        break;
      case EGState.DECAY_SUSTAIN:
        if (this.envelope_counter != EGSustainLevel[this.sustain]) {
          --this.envelope_counter;
          this.envelope_counter &= 0xff;
        }
        break;
...
class WaveformGenerator {
  constructor() {
    ...
    this.set_chip_model(chip.model.MOS6581);
    this.reset();
  }

  set_chip_model(model) {
    if (model == chip.model.MOS6581) {
      this.wave__ST = comboTable.wave6581__ST;
      this.wave_P_T = comboTable.wave6581_P_T;
      this.wave_PS_ = comboTable.wave6581_PS_;
      this.wave_PST = comboTable.wave6581_PST;
    } else {
      this.wave__ST = comboTable.wave8580__ST;
      this.wave_P_T = comboTable.wave8580_P_T;
      this.wave_PS_ = comboTable.wave8580_PS_;
      this.wave_PST = comboTable.wave8580_PST;
    }
  }
...
class ExternalFilter {
  ...
  set_sampling_parameter(pass_freq) {
    this.w0hp = 105;
...
class Filter {
  constructor() {
    ...
    PointPlotter.interpolate(f0_points_6581, this.f0_6581, 1.0);
    PointPlotter.interpolate(f0_points_8580, this.f0_8580, 1.0);
  ...
  clock_one(voice1, voice2, voice3, ext_in) {
    voice1 >>= 7;
    voice2 >>= 7;
  ...
  clock_delta(voice1, voice2, voice3, ext_in, delta_t) {
    voice1 >>= 7;
    voice2 >>= 7;
    if (this.voice3off && !(this.filt & 0x04)) {
      voice3 = 0;
...
const sampling_method = Object.freeze({
  SAMPLE_FAST: 'fast',
  SAMPLE_INTERPOLATE: 'interpolate',
  SAMPLE_RESAMPLE_INTERPOLATE: 'resample_interpolate',
  SAMPLE_RESAMPLE_FAST: 'resample_fast'
});

const f0_points_6581 = new Array(
  [0, 220], [0, 220], [128, 230], [256, 250], [384, 300], [512, 420], [640, 780], [768, 1600], [832, 2300], [896, 3200], [960, 4300], [992, 5000], [1008, 5400], [1016, 5700], [1023, 6000], [1023, 6000], [1024, 4600], [1024, 4600], [1032, 4800], [1056, 5300], [1088, 6000], [1120, 6600], [1152, 7200], [1280, 9500], [1408, 12000], [1536, 14500], [1664, 16000], [1792, 17100], [1920, 17700], [2047, 18000], [2047, 18000]
);

const f0_points_8580 = new Array(
  [0, 0], [0, 0], [128, 800], [256, 1600], [384, 2500], [512, 3300], [640, 4100], [768, 4800], [896, 5600], [1024, 6500], [1152, 7500], [1280, 8400], [1408, 9200], [1536, 9800], [1664, 10500], [1792, 11000], [1920, 11700], [2047, 12500], [2047, 12500]
);

const EGState = Object.freeze({
  ATTACK: 'attack',
  DECAY_SUSTAIN: 'decay_sustain',
  RELEASE: 'release'
});

const EGRateCounterPeriod = Array(
  9, 32, 63, 95, 149, 220, 267, 313, 392, 977, 1954, 3126, 3907, 11720, 19532, 31251
);

// this one seems like overkill... idx +  (idx<<4) should do it...
const EGSustainLevel = Array(
